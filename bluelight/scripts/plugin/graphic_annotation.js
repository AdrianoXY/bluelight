...
function loadWriteGraphic() {
  ...
  span.innerHTML = `<img class="img GSPS" alt="writeGSPS" id="writeGSPS" src="../image/icon/black/gsps_off.png" width="50" height="50">`;
  getByid("icon-list").appendChild(span);

  var span = document.createElement("SPAN");
  span.innerHTML = `<div id="GspsStyleDiv" style="background-color:#30306044;">
...
window.addEventListener("keydown", (KeyboardKeys) => {
  ...
  if (
    (openWriteGSPS || openWriteGraphic) &&
    Graphic_now_choose &&
    (key === 46 || key === 110)
  ) {
...
getByid("writeGSPS").onclick = function () {
  ...
  this.src =
    openWriteGSPS == true
      ? "../image/icon/black/gsps_on.png"
      : "../image/icon/black/gsps_off.png";
...
var Graphic_Annotation_format = `<?xml version="1.0" encoding="UTF-8"?>
<file-format>
      ...
      for (var m = 0; m < PatientMark[n].mark.length; m++) {
        if (PatientMark[n].mark[m].type == "POLYLINE") {
          ...
          for (var o = 0; o < PatientMark[n].mark[m].markX.length; o += 1) {
            if (parseInt(tempMark.markX[o]) >= Max_X)
              Max_X = parseInt(tempMark.markX[o]);
            if (parseInt(tempMark.markX[o]) <= Min_X)
              Min_X = parseInt(tempMark.markX[o]);
          }
          for (var o = 0; o < PatientMark[n].mark[m].markX.length; o += 1) {
            if (equal_TOL(Max_X, parseInt(tempMark.markX[o]), block_size))
              Max_X_list.push(o);
            if (equal_TOL(Min_X, parseInt(tempMark.markX[o]), block_size))
              Min_X_list.push(o);
          }
          for (var o = 0; o < PatientMark[n].mark[m].markY.length; o += 1) {
            if (parseInt(tempMark.markY[o]) >= Max_Y)
              Max_Y = parseInt(tempMark.markY[o]);
            if (parseInt(tempMark.markY[o]) <= Min_Y)
              Min_Y = parseInt(tempMark.markY[o]);
          }
          for (var o = 0; o < PatientMark[n].mark[m].markX.length; o += 1) {
            if (equal_TOL(Max_Y, parseInt(tempMark.markY[o]), block_size))
              Max_Y_list.push(o);
            if (equal_TOL(Min_Y, parseInt(tempMark.markY[o]), block_size))
              Min_Y_list.push(o);
          }
          for (var o = 0; o < PatientMark[n].mark[m].markX.length - 1; o += 1) {
            var x_middle =
              (parseInt(tempMark.markX[o]) + parseInt(tempMark.markX[o + 1])) /
              2;
            var y_middle =
              (parseInt(tempMark.markY[o]) + parseInt(tempMark.markY[o + 1])) /
              2;
            if (
              currY + block_size >= y_middle &&
              currX + block_size >= x_middle &&
              currY < y_middle + block_size &&
              currX < x_middle + block_size
            ) {
      ...
      for (var m = 0; m < PatientMark[n].mark.length; m++) {
        if (PatientMark[n].mark[m].type == "POLYLINE") {
          ...
          for (var o = 0; o < tempMark.markX.length; o += 1) {
            ...
            if (tempMark.RotationAngle && tempMark.RotationPoint)
              [tempX, tempY] = rotatePoint(
                [tempMark.markX[o], tempMark.markY[o]],
                tempMark.RotationAngle,
                tempMark.RotationPoint
              );
            else [tempX, tempY] = [tempMark.markX[o], tempMark.markY[o]];
          ...
          tail = tail.replace(
            "___len___",
            (tempMark.markX.length + tempMark.markY.length) * 4
          );
          tail = tail.replace(
            "___PatternOnColorCIELabValue___",
            "" + SetGraphicColor(PatientMark[n].color)
          );
          tail = tail.replace("___GraphicType___", "POLYLINE");
          tail = setTag(tail, "NumberOfGraphicPoints", 5, true);
          if (tempMark.RotationAngle && tempMark.RotationPoint) {
            var rotation = ("" + Graphic_format_rotation).replace(
              "___RotationAngle___",
              tempMark.RotationAngle
            );
            rotation = rotation.replace(
              "___RotationPoint___",
              "" + tempMark.RotationPoint[0] + "\\" + tempMark.RotationPoint[1]
            );
        ...
        for (var c = 0; c < 5; c++) {
          ...
          temp = setTag(
            ...
            Patient.Study[i].Series[j].SeriesUID,
            true
          );
          temp = setTag(
            temp,
            "AccessionNumber",
            GetViewport().AccessionNumber,
            true
          );
          temp = setTag(
            temp,
            "StudyDescription",
            GetViewport().StudyDescription,
            true
          );
          temp = setTag(temp, "StudyID", GetViewport().StudyID, true);
          temp = temp.replace(
            "___PresentationCreationDate___",
            "" +
              date.getFullYear() +
          ...
          temp = temp.replace(
            "___PresentationCreationTime___",
            "" +
              zero(date.getHours() + 1) +
              zero(date.getMinutes()) +
              zero(date.getSeconds()) +
              "." +
              zero(date.getMilliseconds(), true)
          ); // 093348.775
        }
      }
    }
    ...
    if (PatientMark[n].sop == Patient.Study[i].Series[j].Sop[k].SopUID) {
      for (var m = 0; m < PatientMark[n].mark.length; m++) {
        if (PatientMark[n].mark[m].type == "POLYLINE") {
          ...
          tail = tail.replace(
            "___vm___",
            tempMark.markX.length + tempMark.markY.length
          );
          tail = tail.replace(
            "___len___",
            (tempMark.markX.length + tempMark.markY.length) * 4
          ...
          tail = setTag(
            ...
            (tempMark.markX.length + tempMark.markY.length) / 2,
            true
          );
          if (tempMark.RotationAngle && tempMark.RotationPoint) {
            var rotation = ("" + Graphic_format_rotation).replace(
...
function drawPOLYLINE_Write(obj) {
  ...
  for (var o = 0; o < mark.markX.length; o += 1) {
    ...
    if (
      openWriteGraphic == true ||
      (getByid("GspsPOLYLINE").selected == true && openWriteGSPS == true)
    ) {
      if (mark.RotationAngle && mark.RotationPoint) {
        [x1, y1] = rotatePoint(
          [x1, y1],
      ...
      if (Graphic_now_choose && Graphic_now_choose.mark == mark)
        ctx.strokeStyle = getRGBFrom0xFF(ctx.strokeStyle, false, true);
      ctx.beginPath();
      ctx.arc(
        x1 / 2 + x2 / 2,
        y1 / 2 + y2 / 2,
        parseInt(ctx.lineWidth),
        0,
        2 * Math.PI
      );
      ctx.stroke();
