...
function displayDicomTagsList(viewportNum0) {
  ...
  for (var i = 0; i < GetViewport().DicomTagsList.length; i++) {
    ...
    if (
      GetViewport().DicomTagsList[i][2] &&
      GetViewport().DicomTagsList[i][2].length > 100
    )
      cells.innerHTML =
        ("" + GetViewport().DicomTagsList[i][2]).substring(0, 99) + "...";
    else cells.innerHTML = "" + GetViewport().DicomTagsList[i][2];
