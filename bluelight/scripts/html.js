function html_onload() {
  ...
  function KeyDown(KeyboardKeys) {
    ...
    if (key === 33) {
      jump2UpOrEnd(
        getNowInstance() - parseInt(getAllSop().length / 10) + 1,
        undefined
      );
    } else if (key === 34) {
      jump2UpOrEnd(
  ...
  getByid("ExportButton2").onclick = function () {
    var Export2dcm = function () {
      ...
      link.download = GetViewport()
        .imageId.replace("wadouri:", "")
        .replace("wadors:", "")
        .replace(/^.*(\\|\/|\:)/, "");
      //console.log(link.download);
      if (link.download.includes(".dcm") == false)
        link.download = link.download + ".dcm";
      link.href = GetViewport()
        .imageId.replace("wadouri:", "")
        .replace("wadors:", "");
      link.click();
    };
  ...
  getByid("annotation1").onclick = function () {
    ...
    else getByid("SplitViewportDiv").style.display = "none";
  };
  ...
  getByid("MarkupImg").onclick = function () {
    ...
    var TableSelectOnChange = function () {
      ...
      if (getByid("DICOMTagsSelect").selected == true) displayDicomTagsList();
      else if (getByid("AIMSelect").selected == true) displayAIM();
      else {
        for (var i = 0; i < Viewport_Total; i++) dropTable(i);
      }
    };
  ...
  getByid("WindowLevelSelect").onchange = function () {
    if (getByid("WindowDefault").selected == true) {
      getByid("textWC").value = GetViewport().windowCenterList =
        GetViewport().windowCenter;
      getByid("textWW").value = GetViewport().windowWidthList =
    ...
    for (var i = 0; i < getClass("WindowSelect").length; i++) {
      if (getClass("WindowSelect")[i].selected == true) {
        GetViewport().windowCenterList = getByid("textWC").value = parseInt(
          getClass("WindowSelect")[i].getAttribute("wc")
        );
        GetViewport().windowWidthList = getByid("textWW").value = parseInt(
          getClass("WindowSelect")[i].getAttribute("ww")
        );
        SetWindowWL();
        WindowOpen = true;
        break;
  ...
  getByid("textPlay").onchange = function () {
    ...
    else if (parseInt(getByid("textPlay").value) >= 60)
      getByid("textPlay").value = 60;
    else if (!(parseInt(getByid("textPlay").value) >= 1))
      getByid("textPlay").value = 10;
    PlayTimer();
  };
  ...
  getByid("markAlphaText").onchange = function () {
    if (parseInt(getByid("markAlphaText").value) <= 1)
      getByid("markAlphaText").value = 1;
    else if (parseInt(getByid("markAlphaText").value) >= 100)
      getByid("markAlphaText").value = 100;
    else if (parseInt(getByid("markAlphaText").value) < 100);
  ...
  getByid("markSizeText").onchange = function () {
    if (parseFloat(getByid("markSizeText").value) <= 0.1)
      getByid("markSizeText").value = 0.1;
    else if (parseInt(getByid("markSizeText").value) >= 10)
      getByid("markSizeText").value = 10;
    else if (parseInt(getByid("markSizeText").value) < 10);
...
function changeMarkImg() {
  ...
  if (GetViewport().openMark == true)
    getByid("MarkButton").src = "../image/icon/black/fist0.png";
  else getByid("MarkButton").src = "../image/icon/black/fist1.png";
}

function changeLinkImg() {
  if (openLink == true)
    getByid("unlink").src = "../image/icon/black/b_Link.png";
  else
    getByid("unlink").src =
      "../image/icon/black/b_unlink translation synchronization.png";
}
