...
function compatibleMark(mark) {
  if (!mark.point) {
    ...
    if (mark.markX && mark.markY) {
      var len =
        mark.markX.length >= mark.markY.length
          ? mark.markX.length
          : mark.markY.length;
      for (var o = 0; o < len; o++) {
...
function setImageOrientation2MarkCanvas(viewport, ctx) {
  ...
  if (
    CheckNull(viewport.imageOrientationX) == false &&
    CheckNull(viewport.imageOrientationY) == false &&
    CheckNull(viewport.imageOrientationZ) == false
  ) {
    ctx.setTransform(
      new DOMMatrix([
        viewport.imageOrientationX,
        -viewport.imageOrientationX2,
        0,
        viewport.imagePositionX * viewport.PixelSpacingX,
        -viewport.imageOrientationY,
        viewport.imageOrientationY2,
        0,
        viewport.imagePositionY * viewport.PixelSpacingY,
        viewport.imageOrientationZ,
        viewport.imageOrientationZ2,
        0,
        viewport.imagePositionZ,
        0,
        0,
        0,
        1,
  ...
  //標記支援翻轉
  if (
    viewport.openHorizontalFlip == true &&
    viewport.openVerticalFlip == true
  ) {
    ctx.setTransform(mat.scaleSelf(-1, -1));
    ctx.setTransform(
      mat.translateSelf(
        -parseInt(viewport.imageWidth),
        parseInt(-viewport.imageHeight)
      )
    );
...
function drawSEG(canvas, mark, viewport) {
  ...
  mirrorImage(
    ...
    0,
    0,
    viewport.openHorizontalFlip,
    viewport.openVerticalFlip
  );
  ...
  if (setMarkColor(ctx) == true)
    ctx.fillRect(0, 0, viewport.imageWidth, viewport.imageHeight);
...
function drawTEXT(canvas, mark, viewport) {
  ...
  if (mark.parent.color)
    ctx.strokeStyle = ctx.fillStyle = "" + mark.parent.color;
  try {
    for (var o = 0; o < mark.markX.length; o += 1) {
      ...
      for (var i2 = 0; i2 < lines.length; i2++) {
        var offsetX_temp =
          x1 + 3 * 4 * lines[i2].length > canvas.width
            ? x1 + 3 * 4 * lines[i2].length - canvas.width
            : 0;
        var offsetY_temp =
          y1 + 3 * 4 * lines[i2].length > canvas.height
            ? y1 + 3 * 4 * lines[i2].length - canvas.height
            : 0;
      ...
      if (mark.GSPS_Text && o == 0) {
        ...
        for (var i2 = 0; i2 < lines.length; i2++) {
          ctx.fillText(
            lines[i2],
            x1,
            y1 - 7 - (lines.length - 1) * 3 * 4 + i2 * 3 * 4
          );
        }
        ctx.globalAlpha = tempAlpha;
      }
...
function drawRTSS(canvas, mark, viewport) {
  ...
  for (var o = 0; o < mark.markX.length; o++) {
    ctx.beginPath();
    var x1 = Math.ceil(
      (mark.markX[o] - viewport.imagePositionX) * viewport.PixelSpacingX
    );
    var y1 = Math.ceil(
      (mark.markY[o] - viewport.imagePositionY) * viewport.PixelSpacingY
    );
    var o2 = o == mark.markX.length - 1 ? 0 : o + 1;
    var x2 = Math.ceil(
      (mark.markX[o2] - viewport.imagePositionX) * viewport.PixelSpacingX
    );
    var y2 = Math.ceil(
      (mark.markY[o2] - viewport.imagePositionY) * viewport.PixelSpacingY
    );
...
function drawTextAnnotationEntity(canvas, mark, viewport) {
  ...
  for (var o = 0; o < mark.markX.length; o++) {
    ...
    var ang =
      Math.atan2(
        mark.markY[o] - mark.markY[o + 1],
        mark.markX[o] - mark.markX[o + 1]
      ) +
      Math.PI / 2;
    ctx.rotate(ang);
    ctx.moveTo(0, 0);
...
function getMarkSize(MarkCanvas, sizeCheck) {
  ...
  var lineWid =
    (2 * parseFloat(MarkCanvas.width)) / parseFloat(Css(MarkCanvas, "width"));
  if (sizeCheck == true && lineWid <= 0) {
    lineWid =
      parseFloat(Css(MarkCanvas, "width")) / parseFloat(MarkCanvas.width);
    if (lineWid <= 1.5) lineWid = 1.5;
    lineWid *= Math.abs(
      parseFloat(MarkCanvas.width) / parseFloat(Css(MarkCanvas, "width"))
    );
...
function displayMark(viewportNum0) {
  ...
  var viewport = GetViewport(viewportNum),
    MarkCanvas = GetViewportMark(viewportNum);
  ...
  var patientMark_enable = patientMark_all.filter((M) =>
    checkMark2(viewport.SeriesInstanceUID, M)
  );
  ...
  for (var Mark of patientMark_enable) {
    for (var mark of Mark.mark) {
      mark.parent = Mark;
      if (mark.type == "XML_mark")
        drawXML_mark(MarkCanvas, mark, Mark.showName);
      else if (mark.type == "TEXT") drawTEXT(MarkCanvas, mark, viewport);
      else if (mark.type == "POLYLINE")
        drawPOLYLINE(MarkCanvas, mark, viewport);
      else if (mark.type == "INTERPOLATED")
        drawINTERPOLATED(MarkCanvas, mark, viewport);
      ...
      else if (mark.type == "TwoDimensionPolyline")
        drawTwoDimensionPolyline(MarkCanvas, mark, viewport);
      else if (mark.type == "TwoDimensionMultiPoint")
        drawTwoDimensionMultiPoint(MarkCanvas, mark, viewport);
      else if (mark.type == "TwoDimensionEllipse")
        drawTwoDimensionEllipse(MarkCanvas, mark, viewport);
    }
  }
  ...
  for (var Mark of patientMark_enable) {
    for (var mark of Mark.mark) {
      ...
      else if (mark.type == "TextAnnotationEntity")
        drawTextAnnotationEntity(MarkCanvas, mark, viewport);
    }
  }
